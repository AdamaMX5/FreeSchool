...
// Desktop: a lesson's contents stacked in a comfortable reading column.
export default function ContentView({
  ...
  return (
    <div>
      ...
      <h2 className="mb-2 text-2xl font-semibold">{lesson.name}</h2>
      {lesson.description && <p className="mb-6 text-neutral-400">{lesson.description}</p>}
        ...
        <div className="flex max-w-3xl flex-col gap-8">
          {contents.map((c) => (
            <article key={c.id}>
              ...
              {c.text && <div className="mt-3"><Markdown text={c.text} /></div>}
            </article>
          ))}
        </div>
      )}
